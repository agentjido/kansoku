defmodule KansokuExample.JournalRun do
  @moduledoc """
  Small host-owned worker loop that drains Jizoku journal attempts.

  The example app uses Jizoku's pull-based journal runtime. This worker gives
  ...
  def init(opts) do
    state = %{
      owner_id: Keyword.get(opts, :owner_id, "kansoku-example-journal-run"),
  ...
  defp drain_once(state) do
    case Jizoku.execute_next(owner_id: state.owner_id) do
      ...
      {:error, reason} ->
        Logger.warning("Kansoku example journal drain failed",
