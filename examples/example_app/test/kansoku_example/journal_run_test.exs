defmodule KansokuExample.JournalRunTest do
  use ExUnit.Case, async: false

  alias Ecto.Adapters.SQL.Sandbox
  alias KansokuExample.Repo
  alias KansokuExample.RuntimeSpecDemo
  ...
  test "records inspectable dynamic work overlays for paused runs" do
    ...
    assert {:ok, updated_run} =
             Jizoku.record_dynamic_work(run.run_id, %{
  ...
  test "starts the runtime spec demo through the host action registry" do
    ...
    assert started_run.workflow == "Elixir.KansokuExample.Workflows.CompletedCheckout"
  ...
  test "starts the saga checkout DSL workflow through Kansoku" do
    {:ok, started_run} =
      Kansoku.Runs.start_workflow(
        SagaCheckout,
        %{"order_id" => "order-saga-start", "customer_id" => "cust_demo"}
      )

    assert started_run.run_id
    assert started_run.workflow == "Elixir.KansokuExample.Workflows.SagaCheckout"
  ...
  test "example seed creates a dynamic work overlay demo run" do
    ...
    assert {:ok, runs} = Jizoku.list_runs()

    assert %{run_id: run_id} =
             Enum.find(
               runs,
               &(&1.workflow == "Elixir.KansokuExample.Workflows.PausedCheckout")
             )

    assert {:ok, graph} = Jizoku.inspect_run_graph(run_id)
  ...
  defp await_status(run_id, _expected_status, 0), do: Jizoku.inspect_run(run_id)
  ...
  defp drain_run(run_id, attempts_remaining) do
    case Jizoku.inspect_run(run_id) do
      ...
      {:ok, _run} ->
        _result = Jizoku.execute_next(owner_id: "kansoku-example-test")
