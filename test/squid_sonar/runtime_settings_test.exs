defmodule SquidSonar.RuntimeSettingsTest do
  ...
  test "projects only allowlisted runtime metadata" do
    ...
    projection = RuntimeSettings.project(settings, client: FakeSquidieClient)
  ...
  test "reports a safe needs-attention status without exposing the client error" do
    FakeSquidieClient.put_list_runs({:error, {:missing_config, "/private/path", "secret"}})

    projection = RuntimeSettings.project(%{}, client: FakeSquidieClient)
    json = RuntimeSettings.json(projection)
