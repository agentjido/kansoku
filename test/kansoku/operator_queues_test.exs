defmodule Kansoku.OperatorQueuesTest do
  ...
  test "drops stale paused summaries that no longer have a manual boundary" do
    FakeJizokuClient.put_list_runs({:ok, [summary("resolved-run", :paused, "default")]})
  ...
  test "projects cron declarations from Jizoku workflow modules" do
  ...
  test "returns a safe error when listing or redaction fails" do
    FakeJizokuClient.put_list_runs({:error, {:missing_config, [:journal_storage]}})
  ...
  test "fails closed when the manual queue visibility policy is invalid" do
    FakeJizokuClient.put_list_runs({:ok, [summary("restricted-run", :paused, "default")]})
