defmodule SquidSonarWeb.AssetsTest do
  ...
  test "serves packaged CSS for the current digest" do
    ...
    assert conn.resp_body =~ ".squid-sonar-run-tabs"
    assert conn.resp_body =~ ".squid-sonar-run-tab:focus-visible"
    assert conn.resp_body =~ ".squid-sonar-timeline-list"
    assert conn.resp_body =~ ".squid-sonar-attempt-list"
