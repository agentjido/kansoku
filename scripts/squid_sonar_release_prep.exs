defmodule SquidSonar.ReleasePrepScript do
  ...
  def main(args) do
    ...
    if opts[:notes_only] do
      ...
      IO.puts("Prepared SquidSonar #{version} release notes.")
    else
      ...
      IO.puts("Prepared SquidSonar #{version} release metadata.")
    end
  end
  ...
  defp single_version!(_args) do
    fail!(
      "Expected exactly one version argument, for example: " <>
        "elixir scripts/squid_sonar_release_prep.exs 0.2.1"
    )
  end
...
SquidSonar.ReleasePrepScript.main(System.argv())
