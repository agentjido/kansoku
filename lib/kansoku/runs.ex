defmodule Kansoku.Runs do
  @moduledoc """
  Read boundary for Jizoku workflow runs.

  LiveViews should call this module instead of calling `Jizoku` directly.
  ...
  def list_runs(filters \\ [], opts \\ []) when is_list(filters) and is_list(opts) do
    client = client(opts)
    jizoku_opts = Keyword.get(opts, :jizoku, [])

    with {:ok, runs} <- client.list_runs(filters, jizoku_opts) do
  ...
  def get_run(run_id, opts \\ []) when is_list(opts) do
    ...
    visibility_actor = Keyword.get(opts, :visibility_actor, "kansoku")
    visibility_policy = Keyword.get(opts, :visibility_policy, :auditor)
    controls_allowed? = Keyword.get(opts, :controls_allowed?, true)

    with {:ok, snapshot} <- client.inspect_run(run_id, jizoku_opts),
         {:ok, graph} <- client.inspect_run_graph(run_id, jizoku_opts),
         {:ok, explanation} <- client.explain_run(run_id, jizoku_opts),
         {:ok, timeline, timeline_partial?} <-
           load_timeline(client, run_id, jizoku_opts, snapshot),
         {:ok, visible_snapshot} <-
  ...
  def cancel_run(run_id, opts \\ []) when is_list(opts) do
    client = client(opts)
    jizoku_opts = Keyword.get(opts, :jizoku, [])
    client.cancel(run_id, jizoku_opts)
  end
  ...
  def resume_run(run_id, attrs, opts \\ []) when is_map(attrs) and is_list(opts) do
    client = client(opts)
    jizoku_opts = Keyword.get(opts, :jizoku, [])
    client.resume(run_id, attrs, jizoku_opts)
  end
  ...
  def approve_run(run_id, attrs, opts \\ []) when is_map(attrs) and is_list(opts) do
    client = client(opts)
    jizoku_opts = Keyword.get(opts, :jizoku, [])
    client.approve(run_id, attrs, jizoku_opts)
  end
  ...
  def reject_run(run_id, attrs, opts \\ []) when is_map(attrs) and is_list(opts) do
    client = client(opts)
    jizoku_opts = Keyword.get(opts, :jizoku, [])
    client.reject(run_id, attrs, jizoku_opts)
  end
  ...
  def replay_run(run_id, opts \\ []) when is_list(opts) do
    client = client(opts)
    jizoku_opts = Keyword.get(opts, :jizoku, [])
    client.replay(run_id, jizoku_opts)
  end
  ...
  def start_spec(spec, payload, opts \\ []) when is_list(opts) do
    ...
    client.start_spec(spec, normalize_spec_payload(spec, payload), jizoku_opts)
  end

  @doc """
  Starts a run from a host-provided Jizoku workflow module.
  """
  ...
  def start_workflow(workflow, payload, opts \\ []) when is_atom(workflow) and is_list(opts) do
    ...
    client.start(workflow, normalize_workflow_payload(workflow, payload), jizoku_opts)
  end
  ...
  defp client(opts) do
    Keyword.get(
      opts,
      :client,
      Application.get_env(:kansoku, :jizoku_client, Kansoku.JizokuClient)
    )
  end
