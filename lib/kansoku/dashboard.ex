defmodule Kansoku.Dashboard do
  @moduledoc """
  ...
  @type t :: %__MODULE__{
          runs: [Kansoku.Runs.RunSummary.t()],
          statuses: [atom()],
  ...
  def load(opts \\ []) do
    ...
    runs_opts = Keyword.take(opts, [:client, :jizoku, :visibility_actor, :visibility_policy])
  ...
  def resolve_run_prefix(prefix, opts \\ []) when is_list(opts) do
    with {:ok, normalized_prefix} <- normalize_resolver_prefix(prefix),
         {:ok, runs} <-
           Runs.list_runs(
             [limit: Keyword.get(opts, :limit, @default_limit)],
             Keyword.take(opts, [:client, :jizoku, :visibility_actor, :visibility_policy])
           ) do
  ...
  defp manual_action_ids(_manual, opts, loaded_at) do
    manual_opts =
      opts
      |> Keyword.take([:client, :jizoku, :visibility_actor, :visibility_policy])
