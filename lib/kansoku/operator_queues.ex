defmodule Kansoku.OperatorQueues do
  @moduledoc """
  Builds operator-facing queues from Jizoku's public read models and
  ...
  def list_manual_actions(opts \\ []) when is_list(opts) do
    runs_opts = Keyword.take(opts, [:client, :jizoku])
  ...
  defp visible_snapshot(run_id, opts) do
    ...
    with {:ok, snapshot} <- client.inspect_run(run_id, jizoku_opts) do
      Jizoku.ReadModel.Visibility.redact(snapshot, actor, policy)
    end
  end
