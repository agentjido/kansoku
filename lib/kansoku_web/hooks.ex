defmodule KansokuWeb.Hooks do
  @moduledoc false

  import Phoenix.Component, only: [assign: 3]

  @typep session :: %{required(String.t()) => term()}

  @doc """
  Copies Kansoku session values into LiveView assigns.
  """
  @spec on_mount(:default, map(), session(), Phoenix.LiveView.Socket.t()) ::
  ...
  def on_mount(:default, _params, session, socket) do
    socket =
      ...
      |> assign(
        :control_actor,
        Map.get(session, "control_actor", Kansoku.Router.default_control_actor())
      )
