defmodule KansokuWeb.CoreComponents do
  @moduledoc false
  ...
  def operator_nav(assigns) do
    ~H"""
    <nav class="kansoku-operator-nav" aria-label="Kansoku navigation">
  ...
  def copy_button(assigns) do
    ...
    <button
      id={@id}
      type="button"
      class="kansoku-copy-button"
      phx-hook="KansokuCopy"
  ...
  def panel_heading(assigns) do
    ~H"""
    <div class={["kansoku-panel-heading", @class]}>
      <div class="kansoku-section-heading-copy">
        <p :if={@eyebrow} class="kansoku-eyebrow">{@eyebrow}</p>
        <h2 :if={@level == :h2}>{@title}</h2>
        <h3 :if={@level == :h3}>{@title}</h3>
        <h4 :if={@level == :h4}>{@title}</h4>
        <p :if={@description} class="kansoku-panel-heading-description">{@description}</p>
      </div>

      <div :if={@actions != []} class="kansoku-panel-tools">
        {render_slot(@actions)}
      </div>
    </div>
    """
  ...
  defp nav_link(assigns) do
    ...
    <.link
      navigate={@href}
      class={["kansoku-operator-nav-link", @current && "is-active"]}
  ...
  def status_badge(assigns) do
    ~H"""
    <span class={["kansoku-badge", "kansoku-badge-#{@status}"]}>
      {@status}
    </span>
    """
  end
  ...
  def graph_mode_badge(assigns) do
    ...
    <span class={[
      "kansoku-badge",
      "kansoku-graph-mode-badge",
      "kansoku-graph-mode-badge-#{@mode}"
    ]}>
  ...
  def flash_messages(assigns) do
    ...
    <div
      :if={@info || @error}
      id="kansoku-flash"
      class="kansoku-flash-stack"
      role={if @error, do: "alert", else: "status"}
      phx-hook="KansokuFlash"
    >
      <div class={[
        "kansoku-flash",
        @info && "kansoku-flash-info",
        @error && "kansoku-flash-error"
      ]}>
        <span>{@info || @error}</span>
        <button
          class="kansoku-flash-close"
          type="button"
  ...
  def status_nav_item(assigns) do
    ~H"""
    <label class={["kansoku-nav-item", @active && "is-active"]}>
      <input type="radio" name="filters[status]" value={@status} checked={@active} />
      <span class="kansoku-nav-label">
  ...
  def theme_switcher(assigns) do
    ~H"""
    <div class="kansoku-theme-switcher" aria-label="Theme">
  ...
  def refresh_button(assigns) do
    ~H"""
    <button
      class="kansoku-icon-button kansoku-refresh"
      type="button"
      phx-click="refresh"
      title="Refresh runs"
  ...
  defp theme_button(assigns) do
    ~H"""
    <button
      class={["kansoku-icon-button", @theme == @value && "is-active"]}
      type="button"
      phx-click="set_theme"
      phx-value-theme={@value}
      data-kansoku-theme={@value}
      title={@label}
      aria-label={@label}
    >
      <svg
  ...
  def runs_panel(assigns) do
    ~H"""
    <section class="kansoku-panel kansoku-runs-panel">
      <.panel_heading
        eyebrow="Recent execution activity across the host runtime"
        title="Workflow runs"
        class="kansoku-runs-panel-heading"
      >
        <:actions>
          <button
      ...
      <div class="kansoku-panel-actions kansoku-runs-panel-filters">
        <div class="kansoku-filter-controls kansoku-filter-controls-primary">
          <label class="kansoku-search kansoku-search-query">
            <span>Search</span>
            <input
              type="search"
              name="filters[query]"
          ...
          <label class="kansoku-search kansoku-search-prefix">
          ...
          <label class="kansoku-select-filter">
            <span>Workflow</span>
        ...
        <div class="kansoku-filter-utility-row">
          <details
            class="kansoku-advanced-filters"
            open={@advanced_filters_open? or advanced_filters_active?(@dashboard.filters)}
          >
            <summary phx-click="toggle_advanced_filters">
              <span>Advanced filters</span>
              <span class="kansoku-advanced-filters-hint">Terminal, time, actions, deadline</span>
            </summary>

            <div class="kansoku-filter-controls kansoku-filter-controls-advanced">
              <label class="kansoku-select-filter">
                <span>Terminal</span>
      ...
      <div
        ...
        class="kansoku-saved-workflows-slot"
      >
  ...
  def runs_table(assigns) do
    ...
    <div class="kansoku-table-wrap">
      <table class="kansoku-table kansoku-runs-table">
        <thead>
          <tr>
            <th>Workflow</th>
            <th>Queue</th>
        ...
        <tbody>
          <tr :for={run <- @runs}>
            <td>
              <div class="kansoku-run-title">
                <.link navigate={run_path(@prefix, run.id)} class="kansoku-run-link">
                  <span id={"run-workflow-#{run.id}"} class="kansoku-primary">
                    {format_workflow(run.workflow)}
                  </span>
                  <span id={"run-id-#{run.id}"} class="kansoku-secondary">{run.id}</span>
                </.link>
                <div class="kansoku-copy-controls">
                  <.copy_button
            ...
            <td>
              <div class="kansoku-deadline-cell">
                <span class={[
                  "kansoku-deadline-badge",
                  "kansoku-deadline-badge-#{deadline_status(run.deadline)}"
                ]}>
                  {format_deadline_status(deadline_status(run.deadline))}
                </span>
                <span :if={deadline_step(run.deadline) != "None"} class="kansoku-deadline-meta">
  ...
  def pagination(assigns) do
    ...
    ~H"""
    <nav class="kansoku-pagination" aria-label="Runs pagination">
      <span class="kansoku-pagination-summary">
        <strong>Recent runs</strong>
        <span>{@filtered_count} matching</span>
        <span>Updated <.timestamp value={@loaded_at} /></span>
      </span>
      <div class="kansoku-pagination-controls">
        <label class="kansoku-page-size">
          <span>Page size</span>
  ...
  def run_detail(assigns) do
    ~H"""
    <section class="kansoku-detail">
      <header class="kansoku-detail-header">
        <div>
          <.link navigate={@prefix <> "/"} class="kansoku-back-link">Back to runs</.link>
          <span class="kansoku-section-label">Run summary</span>
          <div class="kansoku-copy-row">
      ...
      <nav class="kansoku-run-tabs" aria-label="Run detail views">
        <.link
          ...
          class={["kansoku-run-tab", @active_tab == tab && "is-active"]}
        >
          {run_tab_label(tab)}
        </.link>
      </nav>
      ...
      <div
        ...
        class="kansoku-run-tab-panel"
      >
        <section class="kansoku-detail-panel kansoku-summary-json-panel">
          <.panel_heading
          ...
          <pre id="run-summary-json" class="kansoku-workflow-raw-json"><code>{run_summary_json(@detail.summary)}</code></pre>
        </section>

        <div class="kansoku-detail-columns">
          <section class="kansoku-detail-panel">
            <h3>Diagnosis</h3>
        ...
        <%= if deadline = @detail.summary.deadline do %>
          <section class="kansoku-detail-panel kansoku-deadline-panel">
            <h3>Deadline</h3>
      ...
      <section
        ...
        class="kansoku-run-tab-panel kansoku-detail-panel kansoku-timeline-panel"
      >
        <.panel_heading
          title="Timeline"
          description="Timestamped runtime events from Jizoku's visibility-filtered read model."
          level={:h3}
        />

        <p :if={@detail.timeline_partial?} class="kansoku-timeline-notice" role="status">
          Timeline data is partial. Available events were reconstructed from the current run snapshot.
        </p>

        <div :if={timeline_events(@detail) == []} class="kansoku-empty kansoku-tab-empty">
          <h3>No timestamped events available</h3>
          <p>The runtime has not exposed chronological evidence for this run yet.</p>
        </div>

        <ol :if={timeline_events(@detail) != []} class="kansoku-timeline-list">
          <li
            :for={event <- timeline_events(@detail)}
            class="kansoku-timeline-event"
            data-timeline-type={event.type}
          >
            <span class={["kansoku-timeline-marker", "is-#{event_status(event)}"]} />
            <div class="kansoku-timeline-event-body">
              <div class="kansoku-timeline-event-heading">
                <strong>{event.summary}</strong>
                <time datetime={DateTime.to_iso8601(event.occurred_at)}>
                  {format_time(event.occurred_at)}
                </time>
              </div>
              <div class="kansoku-timeline-event-meta">
      ...
      <section
        ...
        class="kansoku-run-tab-panel kansoku-detail-panel"
      >
        <.panel_heading
          title="Attempts"
        ...
        <div :if={@detail.attempts == []} class="kansoku-empty kansoku-tab-empty">
          <h3>No attempts available</h3>
          <p>No dispatch attempts are visible for this run.</p>
        </div>

        <div :if={@detail.attempts != []} class="kansoku-attempt-list">
          <article :for={attempt <- @detail.attempts} class="kansoku-attempt-card">
            <div class="kansoku-attempt-heading">
              <strong>{format_step(map_value(attempt, :step))}</strong>
              <.status_badge status={map_value(attempt, :status)} />
            </div>
            <dl class="kansoku-attempt-meta">
      ...
      >
        <section
          ...
          class="kansoku-detail-panel kansoku-recovery-policy-panel"
        >
          <h3>Recovery policies</h3>
          <p>Read-only policy evidence declared by the workflow and exposed by Jizoku.</p>
          <div class="kansoku-recovery-policy-list">
            <article
              :for={policy <- @detail.recovery_policies}
              class="kansoku-recovery-policy-row"
            >
              <strong>{format_step(policy.step)}</strong>
              <div class="kansoku-recovery-policy-tags">
        ...
        <section
          ...
          class="kansoku-detail-panel kansoku-live-claim-panel"
        >
          <h3>Live claims</h3>
          <p>
          ...
          <div class="kansoku-recovery-policy-list">
            ...
            >
              ...
              <div
                :if={claim.anomalies != []}
                class="kansoku-recovery-policy-tags"
              >
        ...
        >
          ...
          <p>
            Read-only rollback and undo evidence; Kansoku does not execute compensation from this summary.
          </p>
          <div class="kansoku-recovery-policy-list">
            <div
        ...
        <section
          ...
          class="kansoku-detail-panel kansoku-dynamic-work-panel"
        >
          <h3>Dynamic work overlays</h3>
          <p>
            Runtime-authored structure is inspection-only here; Kansoku is not treating
        ...
        <section
          ...
          class="kansoku-detail-panel kansoku-deferred-continuation-panel"
        >
        ...
        <div
          ...
          class="kansoku-empty kansoku-tab-empty"
        >
      ...
      <section
        ...
        class="kansoku-run-tab-panel kansoku-detail-panel"
      >
        <div class="kansoku-workflow-panel-heading">
          <h3>{if @workflow_panel_view == :raw, do: "Raw data", else: "Workflow"}</h3>
        </div>

        <%= if @workflow_panel_view == :raw do %>
          <div class="kansoku-workflow-raw">
            <section class="kansoku-raw-summary">
              <.panel_heading
                title="Run summary JSON"
              ...
              <pre id="run-raw-summary-json" class="kansoku-workflow-raw-json"><code>{run_summary_json(@detail.summary)}</code></pre>
            </section>

            <div class="kansoku-workflow-graph-heading">
              <div class="kansoku-workflow-graph-heading-copy">
                <span class="kansoku-section-label">Public graph payload</span>
                <div class="kansoku-workflow-graph-heading-title">
            ...
            <div class="kansoku-workflow-raw-actions">
              <.copy_button
            ...
            <pre id="run-graph-json" class="kansoku-workflow-raw-json"><code>{raw_graph_inspection_json(@detail.graph_inspection)}</code></pre>
          </div>
        <% else %>
          <%= if @detail.workflow_graph.nodes == [] do %>
            <p class="kansoku-muted-line">No workflow graph loaded.</p>
          <% else %>
            <div class="kansoku-workflow-graph">
              <% layout = workflow_graph_layout(@detail.workflow_graph) %>

              <div class="kansoku-workflow-graph-heading">
                <div class="kansoku-workflow-graph-heading-copy">
                  <span class="kansoku-section-label">Journal-backed runtime</span>
                  <div class="kansoku-workflow-graph-heading-title">
              ...
              <div class="kansoku-workflow-graph-evidence">
                <div class="kansoku-workflow-graph-evidence-item">
              ...
              <div
                class="kansoku-workflow-stage"
                style={workflow_stage_style(layout)}
              >
                <span
                  :for={segment <- layout.segments}
                  class={[
                    "kansoku-workflow-edge-segment",
                    "kansoku-workflow-edge-segment-#{segment.orientation}"
                  ]}
                  style={workflow_segment_style(segment)}
                />
                <span
                  :for={port <- layout.ports}
                  class="kansoku-workflow-port"
                ...
                <article
                  ...
                  class={[
                    "kansoku-workflow-node",
                    "kansoku-workflow-node-#{item.node.status}",
                    item.node.dynamic? && "kansoku-workflow-node-dynamic",
                    compensation_node?(item.node) && "kansoku-workflow-node-compensation",
                    item.node.current? && "kansoku-workflow-node-current",
                    item.node.terminal? && "kansoku-workflow-node-terminal"
                  ]}
                  style={workflow_node_style(item)}
                >
                  <div class="kansoku-workflow-node-main">
                    <span class={[
                      "kansoku-workflow-status-icon",
                      "kansoku-workflow-status-icon-#{item.node.status}"
                    ]} />
                    <strong>{item.node.label}</strong>
                    <span :if={item.node.dynamic?} class="kansoku-section-label">Dynamic</span>
                  </div>
                  <span class="kansoku-workflow-node-status">
                    {format_graph_status(item.node.status)}
                  </span>
                  <%= if recovery = compensation_recovery(item.node) do %>
                    <div
                      class="kansoku-workflow-node-recovery-panel"
                      title={"Rollback #{recovery.status} via #{recovery.callback}"}
                    >
                      <span class="kansoku-workflow-node-recovery-label">Rollback</span>
                      <div class="kansoku-workflow-node-recovery-meta">
                  ...
                  <%= if compensation = compensation_node(item.node) do %>
                    ...
                    >
                      <span class="kansoku-workflow-node-recovery-label">Compensation</span>
                      <div class="kansoku-workflow-node-recovery-meta">
                  ...
                  <%= if deadline = deadline_state(item.node) do %>
                    <div
                      class={[
                        "kansoku-workflow-node-deadline",
                        "kansoku-workflow-node-deadline-#{deadline_status(deadline)}"
                      ]}
                      title={"Deadline #{format_deadline_status(deadline_status(deadline))}"}
                    >
                      <span class="kansoku-workflow-node-deadline-label">Deadline</span>
                      <div class="kansoku-workflow-node-deadline-meta">
  ...
  def run_control_buttons(assigns) do
    ...
    <div class="kansoku-control-buttons">
      <%= if :cancel in @available_actions do %>
        <button
          class="kansoku-control-button kansoku-control-button-danger"
          type="button"
          phx-click="cancel"
      ...
      <%= if :resume in @available_actions do %>
        <button
          class="kansoku-control-button kansoku-control-button-primary"
          type="button"
          phx-click="resume"
          phx-value-run-id={@detail.summary.id}
        >
      ...
      <%= if :approve in @available_actions do %>
        <button
          class="kansoku-control-button kansoku-control-button-success"
          type="button"
          phx-click="approve"
          phx-value-run-id={@detail.summary.id}
        >
