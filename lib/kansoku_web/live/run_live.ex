defmodule KansokuWeb.RunLive do
  @moduledoc """
  ...
  def mount(_params, _session, socket) do
    {:ok,
     ...
     |> assign(
       :control_actor,
       Map.get(socket.assigns, :control_actor, Kansoku.Router.default_control_actor())
     )
    ...
    >
      <header class="kansoku-topbar">
        <.link navigate={@prefix <> "/"} class="kansoku-brand kansoku-brand-link">
          <div>
            <p class="kansoku-eyebrow">Run detail</p>
            <h1>Kansoku</h1>
          </div>
        </.link>
  ...
  defp control_attrs(socket) do
    %{actor: Map.get(socket.assigns, :control_actor, Kansoku.Router.default_control_actor())}
  end
