defmodule KansokuWeb.PageLive do
  @moduledoc """
  ...
  def mount(params, _session, socket) do
    ...
    socket =
      ...
      |> assign(:page_title, "Kansoku Runtime")
      |> assign(:theme, :system)
  ...
  def handle_event("jump_to_run", %{"jump" => %{"run_id" => prefix}}, socket) do
    resolver_opts = [
      visibility_actor: Map.get(socket.assigns, :visibility_actor, "kansoku"),
    ...
    >
      <header class="kansoku-topbar">
        <.link navigate={@prefix <> "/"} class="kansoku-brand kansoku-brand-link">
          <div>
            <p class="kansoku-eyebrow">Runtime dashboard</p>
            <h1>Kansoku</h1>
          </div>
        </.link>
        <div class="kansoku-topbar-actions">
          ...
          <form
            ...
            class={["kansoku-run-jump", @jump_error && "is-invalid"]}
            aria-label="Jump to a recent run"
          >
            <div class="kansoku-run-jump-control">
              <label class="kansoku-run-jump-field" for="kansoku-run-jump-input">
                <span class="kansoku-visually-hidden">Run ID prefix</span>
                <svg
                  aria-hidden="true"
                  viewBox="0 0 24 24"
                  fill="none"
                ...
                <input
                  ...
                  aria-describedby={@jump_error && "kansoku-run-jump-error"}
                />
              </label>
              <button type="submit" class="kansoku-run-jump-submit" title="Jump to run">
                Go
              </button>
            </div>
            <span
              :if={@jump_error}
              id="kansoku-run-jump-error"
              class="kansoku-jump-error"
              role="alert"
            >
              {@jump_error}
            </span>
          ...
          <button
            ...
            class="kansoku-control-button kansoku-control-button-secondary kansoku-start-workflow-button"
          >
            Start workflow
          </button>
      ...
      <% else %>
        <div id="workflow-runs" class="kansoku-content">
          <input
            id="kansoku-filter-toggle"
            class="kansoku-filter-toggle-input"
            type="checkbox"
          />

          <form phx-change="filter" phx-submit="filter">
            <label class="kansoku-filter-toggle" for="kansoku-filter-toggle">
              <span class="kansoku-filter-toggle-icon" aria-hidden="true">
                <span></span>
                <span></span>
                <span></span>
              </span>
            ...
            <section class="kansoku-workspace">
              <aside class="kansoku-sidebar" aria-label="Status inventory">
                <div class="kansoku-sidebar-heading">
                  <h2>Status</h2>
                </div>
                <.status_nav_item
                  status={:all}
  ...
  defp runtime_spec_drawer(assigns) do
    ~H"""
    <button
      class="kansoku-runtime-spec-backdrop"
      type="button"
    ...
    <aside
      class="kansoku-runtime-spec-drawer"
      role="dialog"
      aria-modal="true"
      aria-labelledby="kansoku-runtime-spec-title"
    >
      <header class="kansoku-runtime-spec-drawer-header">
        <div>
          <p class="kansoku-eyebrow">Host-approved workflow</p>
          <h2 id="kansoku-runtime-spec-title">Start workflow</h2>
        </div>
        <button
          class="kansoku-runtime-spec-close"
          type="button"
      ...
      <div class="kansoku-runtime-spec-drawer-body">
        <div :if={@start_error} class="kansoku-alert" role="alert">
          <h2>Workflow start failed</h2>
          <p>{@start_error}</p>
        </div>

        <form
          class="kansoku-runtime-spec-form"
        ...
        >
          ...
          <section class="kansoku-runtime-spec-summary" aria-label="Runtime spec boundary">
            <p>
  ...
  defp runtime_spec_catalog_spec(workflow) when is_atom(workflow) and not is_nil(workflow) do
    case Jizoku.Workflow.to_spec(workflow) do
      {:ok, spec} -> spec
