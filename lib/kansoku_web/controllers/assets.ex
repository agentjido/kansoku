defmodule KansokuWeb.Assets do
  @moduledoc false

  use Phoenix.Controller, formats: []

  @static_path Application.app_dir(:kansoku, "priv/static")
  @external_resource css_path = Path.join(@static_path, "kansoku.css")
  ...
  const script = document.querySelector("script[data-kansoku-client]");
  ...
  const themeStorageKey = "kansoku-theme";
  ...
  const applyTheme = (theme) => {
    ...
    document.querySelectorAll(".kansoku-shell").forEach((shell) => {
      shell.classList.remove(
        "kansoku-theme-system",
        "kansoku-theme-light",
        "kansoku-theme-dark"
      );
      shell.classList.add(`kansoku-theme-${theme}`);
    });
  };
  ...
  document.addEventListener("click", (event) => {
    const button = event.target.closest("[data-kansoku-theme]");
    if (!button) return;

    const theme = button.dataset.kansokuTheme;
    storeTheme(theme);
    applyTheme(theme);
  });
  ...
  window.kansokuLiveSocket = liveSocket;
  """
  @js_digest String.slice(Base.encode16(:crypto.hash(:md5, @js), case: :lower), 0, 8)

  @doc """
  Returns the cache digest for Kansoku's packaged CSS asset.
  """
  @spec digest() :: String.t()
  def digest, do: @css_digest

  @doc """
  Returns the cache digest for Kansoku's packaged JavaScript asset.
  """
  ...
  @doc """
  Serves Kansoku's packaged CSS asset when the digest matches.
  """
  ...
  @doc """
  Serves Kansoku's packaged JavaScript asset when the digest matches.
  """
